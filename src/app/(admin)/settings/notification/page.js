...
import { createAppNotification, deleteAppNotification, fetchAppNotifications, updateAppNotification } from "@/lib/api";
...
export default function SettingNotificationPage() {
  ...
  useEffect(() => {
    try {
      ...
      if (raw) {
        ...
        if (flash && flash.scope === "settings_notification" && flash.message) {
  ...
  async function loadItems(page = 1) {
    ...
    setError("");
    try {
      const res = await fetchAppNotifications({ page, per_page: 20 });
      setItems(res?.data || []);
  ...
  async function saveItem() {
    if (!form.unit_name || !form.url) return;
    if (!editing?.id && !form.image) {
      addToast("Gambar wajib diisi", "error");
      return;
    }
    setSaving(true);
    try {
      if (editing?.id) {
        await updateAppNotification(editing.id, form);
      } else {
        await createAppNotification(form);
      }
      setModalOpen(false);
      setEditing(null);
      resetForm();
      try {
        localStorage.setItem(
          "admin_flash",
          JSON.stringify({
            scope: "settings_notification",
            type: "success",
            message: "Data notification berhasil disimpan",
          })
        );
      } catch {}
      ...
      addToast(err?.message || "Gagal menyimpan notification", "error");
    } finally {
      setSaving(false);
    }
  }
  ...
  async function confirmDelete() {
    ...
    try {
      await deleteAppNotification(deleteTarget.id);
      try {
        localStorage.setItem(
          "admin_flash",
          JSON.stringify({
            scope: "settings_notification",
            type: "success",
            message: `Data "${deleteTarget?.unit_name || "Notification"}" berhasil dihapus`,
          })
        );
      } catch {}
      if (typeof window !== "undefined") {
      ...
      addToast(err?.message || "Gagal menghapus notification", "error");
    } finally {
      setDeleteOpen(false);
      setDeleteTarget(null);
    }
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-xl font-semibold text-black">Notification</h1>
          <p className="text-sm text-zinc-700">Kelola notifikasi aplikasi.</p>
        </div>
        <AddButton label="Tambah Notification" onClick={openCreate} />
      </div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full table-fixed text-sm">
            ...
            <tbody>
              {loading ? (
                <tr>
                  <td colSpan={5} className="px-3 py-8 text-center text-zinc-600">Memuat...</td>
                </tr>
              ) : error ? (
                <tr>
                  <td colSpan={5} className="px-3 py-8 text-center text-red-600">{error}</td>
                </tr>
              ) : (items || []).length === 0 ? (
                <tr>
                  <td colSpan={5} className="px-3 py-8 text-center text-zinc-600">Tidak ada data</td>
                </tr>
              ) : (
                (items || []).map((it) => {
      ...
      <Modal
        ...
        title={editing ? "Edit Notification" : "Tambah Notification"}
        className="max-w-xl"
      >
        <div className="space-y-5">
          <div className="grid grid-cols-1 gap-4">
            ...
            <div>
              <label className="text-xs font-medium text-zinc-700">Gambar {editing ? "(opsional)" : "(wajib)"}</label>
              <div className="mt-1 flex items-center gap-3">
                {(() => {
                  ...
                  return preview ? (
                    <Image
                      src={preview}
                      alt={form.unit_name || editing?.unit_name || ""}
                      width={48}
                      height={48}
                      unoptimized
                      className="rounded-md object-cover border border-black/10"
          ...
          <div className="flex items-center justify-end gap-2">
            ...
            <Button onClick={saveItem} disabled={saving || !form.unit_name || !form.url || (!editing?.id && !form.image)}>
